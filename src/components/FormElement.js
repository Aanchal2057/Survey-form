...
const FormElement = ({element, index, createQuestion, createForm}) => {
  ...
  const [radioButtonList, setradioButtonList] = useState([
    { id: uuidv4(), name: "items 1" },
    { id: uuidv4(), name: "items 2" },
    { id: uuidv4(), name: "items 3" },
  ]);
        ...
        case "boolean":
          ...
          break
            case "file":
              ...
              case "radiogroup":
                const selected = [];
                radioButtonList.forEach((list) =>
                  selected.push({ value: list.name, text: list.name })
                );
                _formElement.choices = selected;
  ...
  const removeRadioItem = (id) => {
    if (radioButtonList.length > 2) {
      const newList = radioButtonList.filter((list) => list.id !== id);
      setradioButtonList(newList);
    }
  };
          ...
          return (
            <>
              {radioButtonList.map((radio) => (
                <div key={radio.id} className="d-flex align-items-end mb-3">
                  {radioButtonList.length > 2 && (
                    <i
                      type="button"
                      className="bi bi-dash-circle"
                      onClick={() => removeRadioItem(radio.id)}
                    />
                  )}
                  <input
                    ...
                    type="text"
                    value={radio.name}
                    onChange={(e) => updateRadioItem(radio.id, e.target.value)}
                  />
                </div>
              ))}
              <div className="d-inline-flex">
                <button
                  ...
                  onClick={() =>
                    setradioButtonList((prev) => [
                      ...prev,
                      { id: uuidv4(), name: `item${radioButtonList.length + 1}` },
                    ])
                  }
                >
                  <i className="bi bi-plus-circle"></i> Add Item
